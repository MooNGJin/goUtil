...
func groupConsume(group *sarama.ConsumerGroup, wg *sync.WaitGroup, name string) {
...
func consumerGroup() {
	...
	defer group3.Close()
	wg.Add(3)
	go groupConsume(&group1, &wg, "c1")
	go groupConsume(&group2, &wg, "c2")
	go groupConsume(&group3, &wg, "c3")
	wg.Wait()
